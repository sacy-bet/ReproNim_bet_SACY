...
with open('../Data/Male_2subj_url.txt') as url_males:
    males_data = [line.strip() for line in url_males.readlines()]

for subject in range(0,len(males_data)):
    call('curl {0} > ../Data/Set1/{1}_anat.nii'.format(males_data[subject], subject), shell=True)
    
    
    
with open('../Data/Female_2subj_url.txt') as url_females:
    females_data = [line.strip() for line in url_females.readlines()]

for subject in range(0,len(females_data)):
    call('curl {0} > ../Data/Set2/{1}_anat.nii'.format(females_data[subject], subject), shell=True)
